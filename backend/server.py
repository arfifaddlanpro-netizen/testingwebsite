...
@api_router.get("/")
async def root():
    return {"message": "Dr Plumber JB API"}
...
async def submit_contact_form(input: ContactSubmissionCreate):
    try:
        ...
        return ContactSubmissionResponse(
            success=True,
            message="Mesej anda telah dihantar! Kami akan menghubungi anda dengan segera.",
            id=contact_obj.id
        )
    except Exception as e:
        logger.error(f"Error submitting contact form: {e}")
        raise HTTPException(status_code=500, detail="Gagal menghantar mesej. Sila cuba lagi.")
